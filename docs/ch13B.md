...
| 00:05~00:10 | 바이브코딩 가이드 + 프로젝트 현황 확인 |
| 00:10~00:35 | 체크포인트 1: 설계서 최신화 + 확장 테이블 구축 |
| 00:35~00:55 | 체크포인트 2: 댓글 + 태그 기능 구현 |
| 00:55~01:10 | 체크포인트 3: 검증 + 배포 + 문서화 |
| 01:10~01:15 | Google Classroom 제출 |
| 01:15~01:25 | C파일 공개 + 자기 점검 |
...
> **강의 팁**: 기존 블로그가 동작하지 않는 학생이 있으면 Ch12 스타터 코드에서 시작하도록 안내한다. 기존 코드가 동작하는 상태에서 확장을 시작하도록 주의시킨다.
...
> 제출 마감 후 교수가 공통 체크리스트를 공개한다. 블로그 확장은 공통 주제이므로 **필수 품질 기준**으로 자기 점검한다.

**진행 순서** (10분):

| 시간 | 활동 |
|------|------|
| 3분 | 교수가 필수 품질 기준 체크리스트를 화면에 공개 |
| 5분 | 학생이 자기 블로그를 체크리스트로 점검 — 미달 항목 표시 |
| 2분 | 교수가 가장 흔한 누락 항목 1~2개 짚어줌 |
...
**우수 사례 공유**:
- 완성도 높은 블로그 1-2개를 화면에 띄워 동기부여
- AI_LOG.md에서 흥미로운 AI 실수 사례를 공유

**마무리**:
> 이번 학기에 배운 기술 — Next.js, Tailwind CSS, Supabase, Vercel, 그리고 AI 협업 — 을 하나의 블로그에 통합했다. Ch1에서 시작한 블로그가 댓글, 태그, 검색까지 갖춘 완성품이 되었다. 기말고사에서는 Ch8~12 내용이 객관식으로 출제된다. 블로그 프로젝트와 AI 사용 로그를 정리해두면 시험 준비에도 도움이 된다.
