...
- `app/error.tsx` — 새로 생성 (에러 처리 페이지, `"use client"` 필수)
- `app/loading.tsx` — 새로 생성 (로딩 스피너 또는 스켈레톤 UI)
- `components/PostListSkeleton.tsx` — 새로 생성 (게시글 목록 스켈레톤, 선택사항)
- `app/posts/new/page.tsx` — 수정 (폼 유효성 검증 추가)
- `app/layout.tsx` — 수정 (metadata 설정)
