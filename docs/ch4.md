...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | 변수, 자료형, 함수 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | 객체, 배열, 비동기, 모듈 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 더미 API 연동 실습 + 과제 제출 |
...
> **라이브 코딩 시연**: VS Code에서 일반 함수를 화살표 함수로 변환하는 과정을 보여준다. Copilot에게 "이 함수를 화살표 함수로 바꿔줘"라고 요청하는 것도 시연.
...
> **강의 팁**: `map`과 `filter`가 가장 중요하다. React에서 리스트 렌더링(`map`)과 검색/필터링(`filter`)에 매일 사용한다. `reduce`는 이해만 하고 넘어간다.
...
| Promise.then | `fetch(url).then(res => ...)` | 보통 | △ 간단한 경우 |
...
> **강의 팁**: "import 경로가 틀리면 앱이 실행되지 않는다"를 강조. AI가 `import { supabase } from '@/lib/supabase'` 같은 경로를 생성할 때, 실제로 그 파일이 있는지 반드시 확인해야 한다.

---

### AI가 자주 틀리는 JavaScript 패턴

**표 4.7** AI가 자주 틀리는 JavaScript 패턴

| AI 실수 | 올바른 코드 | 이유 |
|---------|-----------|------|
| `var` 사용 | `const` 또는 `let` | 레거시 문법 |
| `.then()` 체이닝 | `async/await` | 가독성, 프로젝트 규칙 |
| `response.ok` 체크 누락 | `if (!response.ok)` 필수 | HTTP 에러 미처리 |
| 존재하지 않는 import 경로 | 실제 파일 경로 확인 | 환각 — 가짜 경로 생성 |
| `==` 비교 연산자 | `===` 사용 (엄격 비교) | 타입 변환 버그 방지 |
| `.json()` 누락 | `await response.json()` | fetch 응답은 자동 파싱되지 않음 |

> **강의 팁**: 이 표를 슬라이드로 보여주고, "Copilot이 생성한 코드에서 이 실수를 찾아 수정하라"고 안내한다.
...
**과제 내용**: 더미 API에서 데이터를 가져와 화면에 표시하기
...
→ 어떤 API인지, 어떤 패턴(async/await vs .then)인지, 에러 처리는 필요한지 불명확. AI가 .then() 체이닝이나 var를 사용할 수 있다.
...
→ 파일 위치, API URL, 사용 패턴, 에러 처리 방식까지 명확하게 지정했다.

**② AI 코드 검증**

코드를 받으면 다음을 확인한다:

- [ ] `async/await` 패턴을 사용했는가? (`.then()` 체이닝이 아닌지)
- [ ] `response.ok` 체크가 있는가?
- [ ] `await response.json()`으로 파싱했는가?
...
> **Copilot 프롬프트**
> "게시글 목록에 userId로 필터링하는 기능을 추가해줘.
> 버튼을 클릭하면 해당 사용자의 글만 보여주도록 해줘.
> filter 메서드를 사용해줘."

<!-- COPILOT_VERIFY: 필터 기능 프롬프트를 실행하고, filter 메서드 사용 여부를 확인해주세요 -->

**④ 배포**

```bash
git add .
git commit -m "과제: 더미 API 연동 + 필터 기능"
git push
```

**⑤ AI 사용 로그 작성**

어떤 프롬프트를 사용했는지, AI가 무엇을 틀렸는지, 어떻게 수정했는지 기록한다.
...
- **다음 주 예고**: Next.js 기초 — 여러 페이지로 구성된 진짜 웹 앱을 만든다

---

## 장 요약

- **`const`**(기본)와 **`let`**(변경 필요 시)만 사용한다. `var`는 사용하지 않는다
- JavaScript 기본 자료형: 문자열, 숫자, 불리언, null, undefined, 객체, 배열
- **템플릿 리터럴**(`` ` ` ``)로 문자열 안에 변수를 삽입한다
- **화살표 함수**(`() => {}`)는 Next.js에서 가장 많이 보는 함수 형태이다
- **배열 메서드**: `map`(변환), `filter`(걸러내기), `find`(찾기)가 핵심이다
- **구조 분해 할당**(`const { title } = post`)과 **스프레드**(`...`)로 데이터를 다룬다
- 서버 요청은 **비동기**이며, **async/await** 패턴으로 처리한다
- **fetch**로 데이터를 가져오고, `response.ok`와 `try-catch`로 에러를 처리한다
- **import/export**로 코드를 파일 단위로 분리한다
- AI가 생성한 코드에서 `var`, `.then()`, `response.ok` 누락, 가짜 import 경로를 반드시 확인한다
...
- [ ] 라이브 코딩용 프로젝트 (1주차 my-first-web)
- [ ] 배열 메서드 시연용 데이터 준비
- [ ] DevTools Console 탭에서 JavaScript 실행 시연

### 수업 후 체크
- [ ] 전원 API 연동 페이지 배포 URL 제출 확인
- [ ] async/await 패턴 사용 여부 확인 (로그에서)
- [ ] AI 코드 검증 체크리스트 활용 여부 확인
- [ ] 미제출자 파악 및 개별 안내
...
다음 장에서는 **Next.js 기초**를 배운다. 지금까지는 하나의 페이지(app/page.js)만 다루었지만, 다음 장에서는 여러 페이지를 가진 게시판 앱을 만든다. App Router, 동적 라우트, Link 컴포넌트 등을 사용하여 게시글 목록/상세/작성 페이지를 구현한다.
