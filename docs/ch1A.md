...
> **강의 팁**: DevTools Console 탭에서 간단한 JavaScript를 직접 실행해보는 시연을 하면 학생들이 흥미를 느낀다. `document.title = "내 페이지"` 같은 한 줄 코드로 페이지 제목이 바뀌는 것을 보여준다.
...
> **강의 팁**: 시연 마지막에 `app/page.js`의 텍스트를 한 줄 수정하고, `git push`한 뒤 Vercel 대시보드에서 자동 배포가 시작되는 것을 보여주면 학생들에게 강한 인상을 준다.
...
**스타터 코드**: `practice/chapter1/starter/` — create-next-app 결과물 + 간단한 프로필 뼈대가 준비되어 있다. B회차에서 이 코드를 기반으로 시작한다.
...
**준비물 체크리스트**:
- [ ] Node.js LTS 설치 파일 (오프라인 대비 USB 준비)
- [ ] GitHub 계정 생성 사전 안내 (수업 전 공지)
- [ ] Vercel 가입 시연용 계정
- [ ] `create-next-app` 실행 시 네트워크 환경 확인
- [ ] B회차 스타터 코드 준비 (`practice/chapter1/starter/`)
