...
2. **Import Git Repository** 에서 `my-blog` 저장소를 찾아 **Import** 클릭
...
배포가 완료되면 `https://my-blog-xxxxx.vercel.app` 형태의 URL이 생성된다. 이 URL을 브라우저에서 열면 아까 localhost:3000에서 보던 것과 동일한 페이지가 나타난다.
