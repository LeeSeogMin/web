...
4. **한 번에 크게 말고 단계로**: (1) 페이지 맵 → (2) 유저 플로우 → (3) UI 스펙 → (4) 파일 구조/설계서 반영 순서.
...
3. **그 다음 구현**: App Router 폴더 구조 → 공통 레이아웃(Header/Footer) → 핵심 페이지 1개씩.
...
3) User Flow 3개: 예약 신청, 마음톡 작성, 마이페이지 확인
...
주의: 아직 코드는 없으니, '이 파일을 읽어라' 같은 존재 가정은 하지 말고 문서만 만들어줘.
```

#### (3) 구현 시작: App Router 뼈대/네비게이션

```text
너는 GitHub Copilot Chat이야. 이제 문서(ARCHITECTURE.md, copilot-instructions.md)를 기준으로 구현을 시작한다.

목표: Next.js App Router의 최소 뼈대(레이아웃/헤더/푸터/라우팅)만 먼저 만든다.
...
[요구 출력]
1) 생성/수정할 파일 경로 목록
2) `app/layout.tsx`에서 공통 레이아웃(헤더/푸터 포함) 뼈대
3) 헤더 네비게이션 항목과 링크(위 페이지)
4) 각 페이지는 일단 "타이틀 + 1문장 설명"만 넣어서 라우팅 확인 가능하게

제약: 디자인 토큰은 copilot-instructions.md를 따르고, shadcn/ui가 없으면 기본 Tailwind로 임시 구현해도 됨.
```
...
아래 프롬프트는 `mind-center` 실습 프로젝트의 “설계서/디자인 의도”를 먼저 굳히기 위한 템플릿이다. (텍스트로 먼저 확정 → VS Code의 Copilot Chat 또는 Copilot Vision/v0에 입력)

```text
너는 GitHub Copilot Chat이고, 내 `mind-center`(공감터 심리상담연구소) 프로젝트의 제품/프론트엔드 설계 파트너야.
기술 스택: Next.js(App Router) + React + Tailwind CSS + shadcn/ui.
목표: 구현 전에 기술문서를 먼저 만들고(ARCHITECTURE.md, copilot-instructions.md), 그 문서를 기준으로 Copilot이 추측 없이 구현할 수 있게 만드는 것.

[현재 상태]
- 현재 상태: 아직 기술문서/코드가 없다(처음 시작)
- 이번에 만들 것(순서대로):
  1) `ARCHITECTURE.md` 초안(디자인 토큰 + Page Map + User Flow)
  2) `copilot-instructions.md` 초안(코딩/디자인 규칙)
  3) Next.js App Router 기준 폴더 구조 설계(필요 페이지 목록)
  4) 최소 UI(헤더/푸터/홈)부터 구현 계획

[사용자/도메인]
- 타깃 사용자: 상담을 예약하려는 고객, 상담사
- 핵심 문제/가치: 신뢰감 있는 정보 제공 + 빠른 예약 + 마음톡(글)로 상담 전 질문 접수

[디자인 의도/토큰]
- 분위기 키워드 3개: 따뜻함 / 신뢰감 / 여백있는 정보
- 컬러(고정): Primary=#8B6B4E, Background=#FBF8F3, Text=#3A2E26, Border=#E8DDD0
- 레이아웃: 상단 헤더 + 섹션형 랜딩 + CTA 중심
- 금지: 네온 컬러, 과한 그라디언트, 과한 그림자

[요구 산출물]
1) Page Map: App Router 경로로 정리 (예: /, /about, /counseling/[slug], /reservation/offline, /reservation/online, /mindtalk, /mindtalk/new, /mindtalk/[id], /login, /signup, /mypage)
2) User Flow: 비로그인/로그인 상태의 주요 플로우 3개 (예약, 마음톡 작성, 마이페이지 확인)
...
| 00:25~00:50 | AI 디자인 도구 + shadcn/ui + 디자인 토큰                   |
| 00:50~01:20 | 라이브 시연: 설계서 작성 + shadcn/ui 설치 + AI 디자인 변환 |
...
지금까지 Next.js로 페이지를 나누고, 상태를 관리하고, 데이터를 가져오는 방법을 배웠다. 그런데 한 가지 빠진 것이 있다. **"무엇을 만들 것인지"를 정하는 과정**이다.

### 7.1.1 설계 없는 바이브코딩의 문제점: "AI 슬롭"과 균일한 UI

AI에게 "게시판 만들어줘"라고 하면 코드가 만들어진다. 문제는, **모든 학생이 똑같은 결과물을 받는다**는 것이다. 같은 프롬프트 -> 같은 AI -> 같은 디자인. 이렇게 만들어진 결과물을 **AI 슬롭**(AI Slop)이라 부른다.
...
> **팁**: "설계 없이 만든 게시판"과 "설계서를 기반으로 만든 게시판"을 비교해보자. 차이가 극명하다.
...
예를 들어 "비로그인 사용자가 마음톡 글을 작성하는 경우":
...
> **팁**: 게시판 앱의 유저 플로우 3가지(글 읽기, 글 쓰기, 내 글 수정)를 직접 그려보자.
...
대부분의 웹앱은 **사용자 역할(role)**이 필요하다. 일반 사용자와 상담사처럼 역할에 따라 접근 가능한 페이지와 수행 가능한 작업이 달라진다. `role` 컬럼은 Ch9(인증)에서 프로필 조회에 활용하고, Ch11(RLS)에서 역할 기반 접근 제어에 사용한다.
...
- 예약 신청: 홈 → 예약 버튼 → 예약 폼 제출 → (로그인 필요 시 리다이렉트) → 예약 완료
- 마음톡 읽기: 마음톡 목록 → 카드 클릭 → 마음톡 상세
- 마음톡 쓰기: 마음톡 목록 → 글쓰기 → (로그인 필요 시 리다이렉트) → 작성 → 제출 → 상세
...
- users: id, email, name, avatar_url, role ('user'|'counselor')
...
- 월요일에 "마음톡 목록 페이지" 작업 → 수요일에 이어서 "마음톡 상세 페이지" 요청 → **Copilot은 월요일 작업을 모른다**
...
- 마지막 작업일: 2026-02-26
- 완료된 작업: 홈 페이지, 헤더/푸터 레이아웃, 마음톡 목록
- 진행 중: 마음톡 상세 페이지 (UI 완료, 데이터 연결 미완)
- 미착수: 예약 페이지, 마이페이지
...
- Tailwind v4에서는 tailwind.config.js 없이 globals.css의 @theme inline으로 설정
...
```text
방금 마음톡 상세 페이지를 완성했어. todo.md에서 해당 항목을 체크하고 진행률을 업데이트해줘.
```
