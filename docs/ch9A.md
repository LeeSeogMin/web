...
|  ②   | AuthContext + useAuth 전역 상태 · Navbar.js UI  |  🤖  |  9.4  |
|  ③   | middleware.js 보호 라우트                        |  🤖  | 9.4.3 |
|  ④   | context.md 업데이트                              |  🤖  |  9.5  |
...
- 전역 상태: AuthContext + useAuth Hook (contexts/AuthContext.js)
- 보호된 라우트: middleware.js에서 /mypage, /posts/new 등 체크
...
- middleware.js는 app/ 안이 아니라 프로젝트 루트에 위치해야 한다
```
...
2. **이메일 로그인**: `signInWithPassword`(이메일) — `lib/auth.js`에 함수를 중앙화한다
...
2. `lib/auth.js` — signInWithEmail, signUpWithEmail, signOut 구현
3. `contexts/AuthContext.js` — AuthProvider + useAuth
4. `components/Navbar.js` — 로그인/로그아웃 UI (기존 헤더에 통합)
5. `app/login/page.js` — 이메일 로그인 UI
6. `app/signup/page.js` — 이메일 회원가입 UI
7. 이메일 로그인 → 로그아웃 → 재로그인 테스트 성공

### Skills 활용 가이드 (B회차 적용)

- `nextjs-basic-check`: `middleware.js` 위치, App Router 경로, 인증 페이지 라우팅 구조를 점검한다.
- `api-safety-check`: 로그인/회원가입 요청의 입력값 검증, 에러 메시지, 실패 처리 흐름을 점검한다.
- 권장 타이밍: `auth.js` + `AuthContext.js` 작성 후 1회.
