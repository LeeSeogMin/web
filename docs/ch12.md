...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | 에러 유형 + 에러 처리 패턴 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | 로딩 상태 + 폼 유효성 검증 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 성능 기초 + 과제 제출 |
...
> **함께 진행**: error.js를 함께 만들고, 의도적으로 에러를 발생시켜 동작을 확인한다
...
게시글 목록에 로딩 상태를 적용하는 완성 예시:

```jsx
// components/PostList.js (로딩 + 에러 처리 완성 버전)
"use client";
...
이 코드는 Ch10의 PostList에 **로딩(스켈레톤)**과 **에러 처리**를 추가한 완성 버전이다. `try-catch-finally` 패턴에 주목한다 — `finally`에서 `setLoading(false)`를 호출하면 성공이든 실패든 로딩이 종료된다.
...
**검증 체크리스트** — 에러 처리와 UX가 올바른지:

- [ ] 빈 제목으로 제출 시 에러 메시지가 표시되는가?
- [ ] 에러가 있는 필드에 빨간 테두리가 표시되는가?
- [ ] 에러를 수정하고 다시 제출하면 에러 메시지가 사라지는가?
- [ ] 네트워크 에러 시 사용자 친화적 메시지가 나오는가?
- [ ] 데이터 로딩 중 스피너 또는 스켈레톤이 표시되는가?
- [ ] error.js가 예상치 못한 에러를 잡아 대체 UI를 보여주는가?
...
게시판의 기능과 UX가 완성되었다. 마지막으로 **성능**과 **SEO**(Search Engine Optimization, 검색엔진 최적화)를 간단히 개선한다.
...
**표 12.8** 흔한 AI 실수 — 에러 처리와 UX

| 실수 패턴 | 증상 | 해결 |
|-----------|------|------|
| error.js에 `"use client"` 누락 | 에러 페이지가 작동하지 않음 | `"use client"` 지시어 추가 |
| `next/image`에 width/height 누락 | 레이아웃 시프트 발생 | width, height 속성 추가 |
| 에러 메시지에 기술 용어 노출 | 사용자 혼란 | `getUserMessage()` 함수로 변환 |
| 모든 에러에 같은 메시지 | 사용자가 문제를 알 수 없음 | 에러 유형별 다른 메시지 |
| 로딩 상태 없이 데이터 fetch | 빈 화면 후 갑자기 내용 표시 | loading.js 또는 useState로 로딩 표시 |
| 폼 유효성을 서버에서만 검증 | 느린 피드백, 서버 부담 | 클라이언트에서 먼저 검증 |
...
**웹앱 테스트 Skill과 함께 사용하기**: MCP 없이도 테스트 Skill만으로 효과를 볼 수 있다. `.github/skills/webapp-testing/SKILL.md`를 추가하면 "테스트 코드 작성해줘"라는 요청에 Playwright E2E 테스트 코드를 일관된 형식으로 생성한다.
...
| 항목 | 확인 |
|------|:----:|
| `app/error.js` — 에러 처리 페이지 | □ |
| `app/loading.js` — 로딩 스피너 또는 스켈레톤 UI | □ |
| 게시글 작성 폼 유효성 검증 (제목 필수, 내용 필수) | □ |
| 에러 메시지가 사용자 친화적인지 | □ |
...
① **에러 처리**: `app/error.js`와 `getUserMessage()` 유틸리티를 프로젝트에 추가한다

② **로딩 UI**: `app/loading.js`와 게시글 목록 스켈레톤 컴포넌트를 추가한다

③ **폼 유효성**: 게시글 작성/수정 폼에 클라이언트 검증을 추가한다

④ **성능**: `next/image`로 이미지를 교체하고, `metadata`를 설정한다

⑤ **최종 테스트**: 전체 흐름을 테스트한다 — 로그인 → 게시글 작성(유효성 검증) → 목록(스켈레톤) → 수정/삭제(권한 에러) → 로그아웃

⑥ **커밋 및 배포**:

```bash
git add .
git commit -m "과제: 에러 처리 + 로딩 UI + 폼 유효성 + 성능 최적화"
git push
```

_전체 프로젝트는 practice/chapter12/ 참고_

### 과제 제출 + 마무리

- 배포 URL + GitHub 저장소 링크 제출
- 제출 확인
- **완성된 게시판 스크린샷** 함께 제출 (로그인 상태 + 게시글 목록)

---

## 장 요약

- 에러는 4종류: **네트워크**, **인증**, **권한**, **유효성** — 각각 다른 메시지와 조치가 필요
- `try-catch`에서 Supabase 에러를 잡고, `getUserMessage()`로 사용자 친화적 메시지로 변환
- **error.js**: Next.js의 파일 기반 에러 처리 — 해당 경로의 에러를 자동으로 잡아 대체 UI 표시
- **loading.js**: Next.js의 파일 기반 로딩 표시 — 서버 컴포넌트 데이터 로딩 중 자동 표시
- **스켈레톤 UI**: `animate-pulse`로 콘텐츠 뼈대를 미리 보여주기 — 빈 화면보다 좋은 경험
- **Optimistic UI**: 서버 응답 전에 성공을 가정하고 UI 먼저 업데이트 (실패 시 롤백)
- **폼 유효성 검증**: 서버에 보내기 전 클라이언트에서 먼저 검사 — 즉각적 피드백
- **에러 메시지 3원칙**: 무엇이 잘못되었는지 + 왜 발생했는지 + 어떻게 해결하는지
- **next/image**: 이미지 자동 최적화 (WebP 변환, 지연 로딩, 리사이징)
- **metadata**: `<head>` 태그에 title, description 자동 반영 — 기본 SEO
- **Agent Skills**: error-handling Skill로 에러 처리·로딩·유효성 검증 규칙을 AI에 자동 적용
- **참고 — 테스트 MCP**: Playwright MCP로 폼 유효성·에러 메시지 표시를 자동 테스트 가능
...
### 준비물
- [ ] 에러 상황 시연 준비 (네트워크 끊기: Chrome DevTools → Network → Offline)
- [ ] 스켈레톤 UI 예시 사이트 (YouTube, Facebook 등 — 로딩 시 스켈레톤 보이는 사이트)
- [ ] error.js, loading.js 예시 코드 준비
- [ ] "에러 메시지 리디자인" 활동 슬라이드 (나쁜 메시지 → 좋은 메시지로 변환하기)
- [ ] 개인 프로젝트 안내 사항 정리 (Ch13 미리보기)

### 수업 후 체크
- [ ] 전원 error.js + loading.js 추가 확인
- [ ] 전원 폼 유효성 검증 동작 확인
- [ ] 전원 배포 URL 제출 확인
- [ ] 미제출자 파악 및 개별 안내
- [ ] 다음 주(Ch13) 개인 프로젝트 시작 안내 — Ch7 설계서 가져오기

---

## 다음 장 예고

게시판이 **완성**되었다. 기능, 보안, UX 모두 갖춘 풀스택 웹 앱이다. Ch13에서는 이 경험을 바탕으로 **나만의 프로젝트**를 만든다. Ch7에서 작성한 설계서(ARCHITECTURE.md)를 꺼내고, Ch8~12에서 배운 기술을 조합하여 원하는 서비스를 구현한다. 최종 결과물은 Vercel에 배포하고 기말 과제로 제출한다.
