...
| 00:05~00:30 | React 핵심 개념: 컴포넌트, JSX, Props |
| 00:30~00:50 | App Router 구조 + 특수 파일 |
| 00:50~01:20 | 라이브 코딩 시연: 동적 라우트 + 내비게이션 |
...
> **Copilot 프롬프트**
> "PostCard 컴포넌트를 만들어줘. title(문자열), content(문자열), date(문자열), author(문자열)를 props로 받고, Tailwind CSS로 카드 형태로 스타일링해줘. Next.js 14 App Router 프로젝트."
...
> **AI 주의사항**: Next.js 15에서 `params`는 **Promise**이므로 반드시 `await`가 필요하다. AI가 `const { id } = params;`(await 없음)로 생성하면 에러가 발생한다. 이것은 Ch2에서 배운 **버전 불일치** 문제의 대표적 사례이다.
...
> "app/posts/[id]/page.js를 만들어줘. Next.js 14 App Router 사용. params는 Promise이므로 await로 id를 추출해줘. 더미 게시글 데이터(id, title, content, author, date)를 표시하고 Tailwind CSS로 기사 스타일 레이아웃 적용."

나쁜 프롬프트는 어떤 폴더에 만들지, 어떤 버전의 Next.js인지, 어떤 데이터를 표시하는지 명시하지 않는다. AI는 Pages Router(구 방식)로 만들거나, params를 await하지 않을 수 있다. **copilot-instructions.md에 Next.js App Router를 명시**해 두면 이 문제가 크게 줄어든다.
...
**게시글 목록/상세/작성 페이지 구현 + 배포**:
1. 목록 페이지 (`/posts`) — 더미 데이터 게시글 카드 리스트, 클릭 시 상세로 이동
2. 상세 페이지 (`/posts/[id]`) — 해당 게시글 내용, 목록으로 돌아가기 링크
