...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | 왜 설계인가 + 페이지 구조 + 와이어프레임 개념 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | AI 와이어프레임 + shadcn/ui + 디자인 토큰 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 설계서 통합 + 과제 제출 |
...
이 장에서 3단계를 모두 실습한다. 핵심은 **①번을 건너뛰지 않는 것**이다. 레퍼런스 없이 바로 코드를 작성하면 AI 슬롭이 된다.
...
> **참고 — 전문가 도구**: Figma MCP(Figma 디자인→코드), Framelink MCP(Figma 프레임 연결), 21st.dev Magic MCP(AI 컴포넌트 생성), Google Stitch(이미지→프로토타입) 등은 디자이너와 협업하는 전문 워크플로우에서 사용한다. 이 수업에서는 다루지 않지만, "이런 것이 있다"는 정도로 알아두면 된다.
...
1교시에 와이어프레임의 개념과 손그림 스케치를 다뤘다. 이제 실제로 AI 도구를 사용하여 스케치를 코드로 변환한다.

### 7.3.3 Copilot Vision으로 스케치를 코드로 변환하기

> **함께 진행**: 1교시에 그린 종이 스케치를 스마트폰으로 촬영한다.
...
AI가 생성한 코드에서 확인할 것:
- 스케치의 레이아웃(상단 네비게이션, 메인 콘텐츠, 하단 푸터)이 반영되었는가?
- Tailwind 클래스가 레이아웃에 적합한가? (`flex`, `grid`, `justify-between` 등)
- 컴포넌트가 적절히 분리되었는가? (Header, PostList, Footer 등)

> 완벽하지 않아도 괜찮다. Copilot Vision의 결과는 **출발점**이다. 여기서 수정을 반복한다.
...
Copilot Chat에서도 비슷한 프롬프트로 동일한 결과를 얻을 수 있다:

> **Copilot 프롬프트**
> "게시판 앱의 메인 페이지를 만들어줘.
> - 상단에 네비게이션 바 (로고 + 글쓰기 버튼 + 로그인 버튼)
> - 게시글 카드 리스트 (제목, 작성자, 작성일, 미리보기)
> - 검색바 (게시글 제목 검색)
> - shadcn/ui의 Card, Input, Button 컴포넌트를 사용
> - Tailwind CSS로 반응형: md 이상 2열 그리드, 모바일 1열"

**v0 출력 코드 읽기 포인트**:
- 어떤 shadcn/ui 컴포넌트를 사용했는가? (`Card`, `CardHeader`, `CardContent` 등)
- Tailwind 반응형 클래스가 적용되었는가? (`md:grid-cols-2` 등)
- 더미 데이터가 어떤 형태인가? (나중에 Supabase 데이터로 교체할 구조인지)
...
**shadcn/ui**는 일반적인 npm 패키지가 아니다. `npm install`로 설치하는 것이 아니라, **컴포넌트 코드를 프로젝트에 직접 복사**한다. 복사된 코드는 내 프로젝트의 일부가 되므로 자유롭게 수정할 수 있다. 이런 방식의 **컴포넌트 라이브러리**(Component Library)는 AI 코딩 시대에 특히 유리하다.
...
`cn()` 함수는 Tailwind 클래스를 조건부로 합치는 유틸리티이다. shadcn/ui 컴포넌트에서 자주 사용된다.
...
이 명령어를 실행하면 `components/ui/` 폴더에 파일이 복사된다.

**표 7.5** shadcn/ui 핵심 컴포넌트
...
`components/ui/button.tsx` 파일을 열어보자. Tailwind 클래스로 스타일이 정의되어 있고, `variant` prop으로 버튼의 모양을 바꿀 수 있다. **이 코드는 내 프로젝트의 일부이므로 자유롭게 수정할 수 있다.**
...
> CSS 변수 값은 HSL 색상 형식이다 (색상, 채도%, 밝기%). 예를 들어 `222.2 47.4% 11.2%`는 어두운 파란색이다.

**색상 팔레트 변경하기**:

shadcn/ui 공식 사이트(ui.shadcn.com/themes)에서 테마를 선택하면 CSS 변수 코드를 복사할 수 있다. `globals.css`의 `:root` 부분을 교체하면 전체 테마가 바뀐다.
...
7.4.4에서 설정한 CSS 변수가 바로 디자인 토큰이다. 이것을 copilot-instructions.md에도 명시하면 AI가 매번 참조한다.
...
**표 7.7** 디자인 프롬프트 5가지 전략

| # | 전략 | 설명 | 예시 |
|:-:|------|------|------|
| ① | **레퍼런스 제시** | 참고할 UI/사이트 이름을 명시한다 | "Notion 스타일의 사이드바" |
| ② | **제약 조건 명시** | 사용할 컴포넌트, 색상 등을 제한한다 | "shadcn/ui Card만 사용, primary 색상 위주" |
| ③ | **반복 다듬기** | 한 번에 완성하지 않고 단계적으로 수정한다 | "간격을 더 넓게" → "폰트를 키워줘" |
| ④ | **부정 프롬프트** | 하지 말아야 할 것을 명시한다 | "그라디언트 사용 금지, 그림자 최소화" |
| ⑤ | **역할 부여** | AI에게 디자인 역할을 지정한다 | "미니멀리스트 UI 디자이너로서 답변해줘" |

핵심은 **③번 반복 다듬기**이다. 디자인은 한 번에 완성되지 않는다. 첫 결과를 보고 "간격이 좁다", "색이 너무 진하다" 같은 구체적인 피드백을 반복한다.

### 7.5.4 좋은 디자인 프롬프트 vs 나쁜 디자인 프롬프트

**표 7.8** 좋은 vs 나쁜 디자인 프롬프트
...
이 프롬프트는 AI에게 아무런 시각적 컨텍스트를 주지 않는다. 결과는 AI 슬롭이 된다.

> **Copilot 프롬프트** (좋은 예)
> "사용자 프로필 페이지를 만들어줘.
> - shadcn/ui Avatar (크기: h-20 w-20), Card, Badge, Button 사용
> - 상단: 아바타 + 이름(text-2xl font-bold) + 이메일(text-muted-foreground)
> - 중단: "내가 쓴 게시글" 목록 (shadcn/ui Card 리스트)
> - 간격: space-y-6, 최대 너비 max-w-2xl mx-auto
> - 반응형: 상단 영역은 모바일에서 세로 스택(flex-col), 데스크톱에서 가로(flex-row)"

두 프롬프트의 결과를 비교해보면, 구체적인 프롬프트가 훨씬 의도에 가까운 코드를 생성한다.

---

### AI가 자주 틀리는 디자인 패턴

**표 7.9** AI가 자주 틀리는 디자인 패턴
...
> **강의 팁**: 이 표를 체크리스트처럼 사용한다. AI가 생성한 UI 코드를 받으면 이 6가지를 먼저 확인하도록 안내한다.
...
1교시에서 페이지 맵과 유저 플로우를, 2교시에서 와이어프레임과 컴포넌트 시스템을 만들었다. 3교시에서는 이 모든 것을 **하나의 설계 문서**로 통합하고, AI가 참조할 수 있는 컨텍스트로 만든다.
...
> 데이터 모델은 완벽하지 않아도 된다. Ch8에서 실제 테이블을 만들 때 수정할 수 있다. 중요한 것은 **"어떤 데이터가 필요한지" 미리 생각해보는 것**이다.
...
2장에서 배운 것처럼, 이 파일이 있으면 **@workspace** 명령어로 Copilot이 프로젝트 전체 설계를 참조할 수 있다.

### 7.6.3 AI 생성 디자인 검증 체크리스트: 접근성, 반응형, 일관성

AI가 만든 UI가 제대로 되었는지 확인하는 체크리스트이다. **이 체크리스트는 Ch8 이후 모든 실습에서 사용한다.**

**표 7.10** AI 디자인 검증 체크리스트

| # | 검증 항목 | 확인 내용 | 확인 방법 |
|:-:|-----------|----------|-----------|
| 1 | **반응형** | 모바일(375px), 태블릿(768px), 데스크톱(1280px)에서 레이아웃이 깨지지 않는가? | DevTools → 반응형 모드 (Ctrl+Shift+M) |
| 2 | **접근성** | 이미지에 alt, 버튼에 의미 있는 텍스트, 충분한 색상 대비가 있는가? | Lighthouse → 접근성 점수 |
| 3 | **일관성** | 모든 페이지에서 동일한 디자인 토큰(색상, 간격, 폰트)을 사용하는가? | 육안 비교 |
...
핵심은 **"@workspace"를 붙이는 것**이다. 이 한 단어가 Copilot에게 프로젝트 전체 파일(ARCHITECTURE.md, copilot-instructions.md 포함)을 탐색하도록 지시한다.

> 설계가 바뀌면 ARCHITECTURE.md와 copilot-instructions.md를 **즉시 업데이트**한다. AI는 기억하지 못하지만, 설계 문서는 기억한다 — 2장에서 배운 원칙과 같다.
...
**제출**: GitHub 저장소에 push + Vercel 배포 URL
...
어떤 앱을 만들지 정한다. 너무 복잡하지 않게 — "데이터를 목록으로 보여주고, 상세 보기, 작성, 수정, 삭제"가 가능한 주제면 충분하다.

**② 페이지 맵 + 유저 플로우 작성**

종이에 페이지 맵을 그리고, 주요 유저 플로우 2~3개를 정리한다.

**③ AI 와이어프레임 생성**

1교시에 그린 스케치를 Copilot Vision에 넣거나, v0에 프롬프트를 입력하여 프로토타입을 생성한다.
...
`globals.css`에서 색상 토큰을 커스터마이징한다.

**⑤ 설계 문서 작성**

ARCHITECTURE.md와 copilot-instructions.md를 작성한다.

**⑥ GitHub push + Vercel 배포**

```bash
git add .
git commit -m "과제: 개인 프로젝트 설계서 + shadcn/ui 테마"
git push
```
...
- 제출 확인
- **잘 만든 설계서 1~2개 화면 공유** (동기부여)
- **다음 주 예고**: 중간고사 — Ch1~7 범위, 코딩 객관식

---

## 장 요약

- **AI 슬롭**: 설계 없이 AI에게 맡기면 모든 프로젝트가 비슷하게 생긴다
- **3단계 디자인 파이프라인**: 탐색(레퍼런스) → 프로토타입(와이어프레임) → 코드(Copilot+shadcn/ui)
- **페이지 맵**으로 필요한 페이지와 URL 구조를 먼저 정리한다
- **유저 플로우**로 사용자의 화면 이동 경로를 미리 설계한다
- **Copilot Vision**: 손그림 스케치를 이미지로 촬영 → Copilot Chat에 드래그 → 코드 변환
- **v0**: 텍스트 프롬프트 → React+Tailwind+shadcn 프로토타입 즉시 생성
...
- [ ] 설계 없이 만든 예시 vs 설계 후 만든 예시 스크린샷 (1교시 동기부여용)

### 수업 후 체크
- [ ] 전원 shadcn/ui 초기화 완료 확인
- [ ] 전원 ARCHITECTURE.md + copilot-instructions.md(Design Tokens 포함) 작성 확인
- [ ] 전원 배포 URL 제출 확인 (설계서가 GitHub에 push되었는지)
- [ ] 미제출자 파악 및 개별 안내
- [ ] 다음 주 중간고사 범위 및 형식 공지 (Ch1~7, 코딩 객관식)

---

## 다음 장 예고

다음 주는 **중간고사**이다. Ch1~7 범위의 코딩 객관식 시험이다. 시험이 끝나면 Part 5 **Supabase 백엔드 통합**으로 넘어간다. 오늘 작성한 설계서(ARCHITECTURE.md)의 데이터 모델을 **진짜 데이터베이스 테이블**로 만드는 것이 Ch8의 핵심이다. 설계가 잘 되어 있을수록 Ch8 이후 작업이 수월해진다.
