...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | 인증 개념 + Google OAuth 설정 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | 인증 구현 + 인증 상태 관리 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 보호된 페이지 + 과제 제출 |

---

# 1교시: 인증 개념과 Google OAuth 설정
...
**검증 체크리스트** — OAuth 설정이 올바른지:

- [ ] Google Cloud Console에 OAuth 2.0 Client ID가 생성되었는가?
- [ ] Client ID와 Client Secret이 Supabase에 입력되었는가?
- [ ] Supabase Callback URL이 Google의 Authorized redirect URIs에 등록되었는가?
- [ ] 테스트 사용자에 본인 이메일이 추가되었는가?
...
> @supabase/ssr의 createBrowserClient를 사용하고, 로그인 성공 후 /dashboard로 리다이렉트해줘."
...
이 프롬프트로는 이메일+비밀번호 방식인지, 소셜 로그인인지 AI가 알 수 없다. 어떤 Provider(Google/GitHub/Apple)인지도 모른다. 결과적으로 이 수업에서 사용하지 않는 `signInWithPassword` 코드가 나올 가능성이 높다.
...
**검증 체크리스트** — 로그인 흐름이 올바른지:

- [ ] "Google로 로그인" 버튼 클릭 시 Google 로그인 화면이 나오는가?
- [ ] Google 계정 선택 후 앱으로 돌아오는가 (redirect_uri_mismatch 에러 없이)?
- [ ] 로그인 후 이메일이 네비게이션 바에 표시되는가?
- [ ] 로그아웃 버튼 클릭 시 로그인 버튼으로 바뀌는가?
- [ ] 페이지 새로고침 후에도 로그인 상태가 유지되는가?

**표 9.7** 흔한 AI 실수 — 인증 구현

| 실수 패턴 | 증상 | 해결 |
|-----------|------|------|
| `signInWithPassword` 사용 (OAuth 아님) | 이메일+비밀번호 폼 생성 | `signInWithOAuth({ provider: "google" })` 사용 |
| `redirectTo`에 하드코딩된 URL | 로컬에서 동작, 배포 시 실패 | `window.location.origin` 사용 |
| `onAuthStateChange` 리스너 정리 안 함 | 메모리 누수, 중복 이벤트 | `useEffect` return에서 `unsubscribe()` |
| `getSession()` 사용 (deprecated) | 향후 버전에서 제거 예정 | `getUser()` 사용 (Supabase v2+ 권장) |
| 콜백 Route Handler 누락 | 로그인 후 홈으로 돌아오지 않음 | `app/auth/callback/route.js` 생성 |
| AuthProvider를 layout.js에 안 감쌈 | `useAuth()`가 undefined 반환 | 최상위 layout.js에 AuthProvider 추가 |
...
export async function middleware(request) {
  ...
  // config.matcher가 보호된 경로만 필터링하므로,
  // 여기 도달한 요청은 모두 인증이 필요하다
  if (!user) {
...
오늘 수업에서 구현한 Google 로그인 기능을 완성하여 제출한다.
...
| Supabase Auth Provider(Google) 설정 완료 | □ |
| `lib/auth.js` — signInWithGoogle, signOut 구현 | □ |
| `app/auth/callback/route.js` — 콜백 처리 구현 | □ |
| `contexts/AuthContext.js` — AuthProvider + useAuth 구현 | □ |
| `components/Navbar.js` — 로그인/로그아웃 UI 구현 | □ |
| `middleware.js` — 보호된 경로 설정 | □ |
| Google 로그인 → 로그아웃 → 재로그인 테스트 완료 | □ |
| GitHub push + Vercel 배포 | □ |

### 실습 진행

① **설정 확인**: 1교시에서 설정한 Google OAuth + Supabase Provider가 올바른지 확인한다

② **코드 작성**: 2교시에서 작성한 파일들을 프로젝트에 추가한다. Copilot에게 프롬프트를 보내며 코드를 생성하되, 반드시 위의 코드 읽기 가이드와 대조한다.

③ **테스트**: 로컬에서 Google 로그인 → 로그아웃 → 재로그인이 정상 동작하는지 확인한다

④ **Vercel 배포**: 배포 후 배포 URL에서도 Google 로그인이 동작하는지 확인한다. **Google Cloud Console의 Authorized redirect URIs에 배포 URL의 콜백 경로도 추가**해야 한다:
...
```bash
git add .
git commit -m "과제: Google 로그인 구현"
git push
```

_전체 프로젝트는 practice/chapter9/ 참고_

### 과제 제출 + 마무리

- 배포 URL + GitHub 저장소 링크 제출
- 제출 확인
- **로그인 성공 화면 스크린샷** 함께 제출 (이메일이 표시된 네비게이션 바)
...
- Supabase는 **JWT 기반** 인증을 사용하며, `@supabase/ssr`이 쿠키 관리를 자동 처리한다
- **OAuth 소셜 로그인**: 사용자 → Google → Supabase → JWT 발급 → 쿠키 저장
- `signInWithOAuth({ provider: "google" })` 한 줄로 Google 로그인이 시작된다
- `onAuthStateChange`로 로그인/로그아웃 이벤트를 실시간 감지한다
- **AuthContext**: React Context로 인증 상태를 전역 관리, `useAuth()` Hook으로 사용
- **콜백 Route Handler**: `app/auth/callback/route.js`에서 인증 코드 → 세션 교환
- **미들웨어**: 보호된 경로에 비로그인 사용자 접근 시 서버 레벨에서 리다이렉트
- Google Cloud Console과 Supabase 양쪽 설정이 모두 필요하다
- 배포 시 Vercel URL의 콜백 경로도 Google에 등록해야 한다
...
### 수업 후 체크
- [ ] 전원 Google Cloud Console OAuth 설정 완료 확인
- [ ] 전원 Supabase Auth Provider 설정 완료 확인
- [ ] 전원 Google 로그인 성공 확인 (최소 1회)
- [ ] 전원 배포 URL에서도 로그인 동작 확인
- [ ] 전원 배포 URL 제출 확인
- [ ] 미제출자 파악 및 개별 안내
- [ ] 배포 환경에서 Callback URL 미등록으로 실패하는 학생 지원

---

## 다음 장 예고

로그인이 되었으니, 이제 **누가 어떤 글을 썼는지** 구분할 수 있다. Ch10에서는 Supabase 데이터베이스에 게시글을 **생성, 조회, 수정, 삭제**(CRUD)하는 기능을 구현한다. 로그인한 사용자만 글을 쓸 수 있고, 모든 사용자가 글을 읽을 수 있는 게시판을 완성한다.
