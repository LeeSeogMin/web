...
> **강의 팁**: 순회하며 학생들이 최소 4페이지를 설계했는지, 테이블 간 관계(FK)를 정의했는지 확인한다. 기존 블로그 구조를 기반으로 보강하도록 안내한다.
...
> 제출 마감 후 교수가 C파일(모범 설계서)을 화면에 공개한다. 학생은 자기 블로그 설계서와 비교하여 차이점을 찾고 보완한다.

**진행 순서** (15분):

| 시간 | 활동 |
|------|------|
| 3분 | 교수가 모범 설계서의 핵심 구조를 화면 공유로 설명 |
| 5분 | 학생이 자기 설계서와 모범 설계서를 비교 — 빠진 부분 찾기 |
| 5분 | 빠진 부분을 자기 설계서에 추가 |
| 2분 | 교수가 설계서에서 가장 중요한 항목 짚어줌 |
