...
Ch6까지 만든 블로그 프로젝트를 이어서 사용한다. 프로젝트 루트에 아래 파일을 새로 만든다.

- `ARCHITECTURE.md` — 설계서 (페이지 맵 + 컴포넌트 계층 + 데이터 모델)
- `.github/copilot-instructions.md` — 수정 (Design Tokens + Component Rules 섹션 추가)
- `context.md` — 새로 생성 (프로젝트 현재 상태 기록)
- `todo.md` — 새로 생성 (전체 작업 체크리스트)
...
> 제출 마감 후 모범 설계서를 확인한다. 자기 블로그 설계서와 비교해 차이점을 찾고 보완한다.
...
|------|------|
| 3분 | 모범 설계서 핵심 구조 확인 |
| 5분 | 자기 설계서와 모범 설계서를 비교 — 빠진 부분 찾기 |
