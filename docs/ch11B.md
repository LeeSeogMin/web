...
Ch10에서 만든 블로그 프로젝트를 이어서 사용한다. 이 장의 핵심은 Supabase SQL Editor에서 RLS 정책을 작성하는 것이므로, 코드 수정보다 대시보드 작업이 중심이다.

- `supabase/policies.sql` — 새로 생성 (RLS 정책 SQL 기록용, 선택사항)
- Supabase 대시보드 SQL Editor에서 posts + profiles 테이블 RLS 활성화 + 정책 생성
...
> 제출 마감 후 모범 구현을 확인한다. 자기 RLS 정책과 비교해 차이점을 찾고 수정한다.
...
|------|------|
| 3분 | 참고 구현 핵심 구조 확인 |
| 5분 | 자기 RLS 정책 SQL과 참고 구현을 비교 — 다른 부분 찾기 |
