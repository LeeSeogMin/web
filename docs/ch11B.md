...
> **주의**: RLS 테스트를 위해 Supabase MCP의 `--read-only`를 해제해야 할 수 있다. 교수 안내에 따라 진행한다.
...
| 2분 | 교수가 USING vs WITH CHECK 핵심 차이 짚어줌 |
...
**다음 주 예고**:
> 다음 주에는 **에러 처리와 UX 완성**을 배운다. 지금까지 만든 블로그에 에러 페이지, 로딩 UI, 폼 유효성 검증, 이미지 최적화를 추가하여 사용자가 불편함 없이 쓸 수 있는 앱으로 다듬는다.
