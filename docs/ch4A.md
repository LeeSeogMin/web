...
| 00:05~00:25 | 변수, 자료형, 함수 |
| 00:25~00:50 | 객체, 배열, 배열 메서드 |
| 00:50~01:20 | 라이브 코딩 시연: 비동기 + fetch + 모듈 |
...
> **강의 팁**: VS Code에서 일반 함수를 화살표 함수로 변환하는 과정을 보여준다. Copilot에게 "이 함수를 화살표 함수로 바꿔줘"라고 요청하는 것도 시연한다.
...
> **라이브 코딩 시연**: 교수가 공개 API(JSONPlaceholder)를 호출하여 데이터를 콘솔에 출력하고, 화면에 표시하는 과정을 시연한다. fetch → json 파싱 → map으로 리스트 렌더링 순서로 진행한다.
...
- [ ] B회차 스타터 코드 준비 (`practice/chapter4/starter/`)
- [ ] DevTools Console 탭에서 JavaScript 실행 시연 준비
- [ ] 배열 메서드 시연용 데이터 준비
- [ ] 라이브 코딩용 프로젝트 (Ch3 complete 기반)

**수업 후 체크**:
- [ ] 학생들이 const vs let 차이를 이해했는가
- [ ] 화살표 함수를 읽을 수 있는가
- [ ] map, filter 동작을 설명할 수 있는가
- [ ] async/await 패턴의 기본 구조를 이해했는가
