...
> **강의 팁**: 이 표는 B회차 실습에서 AI 코드를 검증할 때 체크리스트로 활용한다. 슬라이드에 띄워두면 좋다.
...
> **강의 팁**: 이 수업에서는 styled-components, CSS Modules 등 다른 방식은 사용하지 않는다. Tailwind CSS만 쓴다.
...
> **강의 팁**: 교수 화면에서 확장 설치를 시연하고, 클래스명 위에 마우스를 올리면 CSS 값이 표시되는 것을 보여준다.
...
> **강의 팁**: `flex` 하나만 추가했을 때, `justify-between`을 추가했을 때, `items-center`를 추가했을 때 변화를 하나씩 보여주면 효과적이다.
...
- [ ] Tailwind 클래스를 보고 CSS 속성을 유추할 수 있는가
- [ ] 반응형 브레이크포인트 개념을 이해했는가
