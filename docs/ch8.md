...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | BaaS 개념 + Supabase 프로젝트 생성 + 대시보드 탐색 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | Next.js 연결 + 환경 변수 + Vercel 배포 설정 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 데이터 모델링 + 테이블 생성 + 과제 제출 |

---

# 1교시: BaaS 개념과 Supabase 시작
...
# 2교시: Next.js와 Supabase 연결

## 8.3 Next.js와 Supabase 연결

1교시에 만든 Supabase 프로젝트를 기존 Next.js 프로젝트에 연결한다.
...
1교시에서 확인한 API 키를 프로젝트에 저장한다. 프로젝트 루트에 `.env.local` 파일을 생성한다:
...
3. **실제 값 입력**: `[프로젝트ID]`와 `eyJhbG...` 부분을 1교시에서 확인한 본인의 값으로 교체한다.

```bash
# .gitignore에 이미 포함되어 있는지 확인
# .env*.local  ← 이 줄이 있어야 한다
```
...
**표 8.7** Supabase 클라이언트 초기화 코드 해석
...
export default function TestPage() {
  ...
  useEffect(() => {
    ...
    // 존재하지 않는 테이블을 조회하여 연결 확인
    supabase.from("_connection_test").select("*").limit(0)
      .then(({ error }) => {
        // 서버가 응답했다면 연결 성공 (테이블 미존재 에러도 서버 응답임)
        // 연결 실패 시에는 "Failed to fetch" 등 네트워크 에러가 발생
...
**표 8.9** 흔한 AI 실수 — Supabase 클라이언트 설정
...
| 환경 변수 수정 후 서버 재시작 안 함 | 이전 값이 계속 사용됨 | `Ctrl+C` → `npm run dev` 재실행 |
...
2교시까지 Supabase 연결이 완료되었다. 이제 **데이터를 저장할 그릇(테이블)**을 만들 차례이다.
...
이 프롬프트로는 AI가 임의로 열 이름과 타입을 정한다. `auth.users`와의 연결 없이 별도 `users` 테이블을 만들거나, `int` 타입의 ID를 생성할 가능성이 높다. Supabase의 인증 시스템과 맞지 않는 테이블이 나온다.

> **좋은 프롬프트** (Copilot 프롬프트)
...
차이가 보이는가? 좋은 프롬프트는 **테이블 이름, 열 이름, 참조 관계, 제외 사항(RLS)**을 모두 명시한다. 7장에서 작성한 ARCHITECTURE.md의 데이터 모델을 프롬프트에 그대로 반영하면 된다.
...
**검증 체크리스트** — 테이블이 올바르게 생성되었는지:

- [ ] Table Editor에 `profiles`, `posts` 두 테이블이 보이는가?
- [ ] `profiles.id`의 타입이 `uuid`인가?
- [ ] `posts.user_id`에 외래 키 아이콘이 표시되는가?
- [ ] `posts.title`과 `posts.content`에 "not null" 제약이 있는가?
...
| `on delete cascade` 누락 | 사용자 삭제 시 고아 레코드 발생 | 외래 키에 `on delete cascade` 추가 |
| 테이블 생성 순서 오류 | 참조 대상 테이블이 아직 없음 | `profiles` → `posts` 순서로 생성 |
| `not null` 빠뜨림 | 빈 제목/내용의 게시글 생성 가능 | 필수 필드에 `not null` 추가 |

## 8.5 과제: Supabase 프로젝트 설정 + 테이블 생성 + 배포

### 과제 안내

오늘 수업에서 진행한 내용을 정리하여 제출한다.
...
① **Supabase 설정 확인**: 1~2교시에서 설정한 내용이 모두 완료되었는지 확인한다

② **테이블 생성**: 3교시에서 작성한 SQL을 SQL Editor에서 실행한다. 7장 설계서의 데이터 모델과 다른 부분이 있다면 설계서에 맞게 수정한다.

③ **코드 커밋 및 배포**:

```bash
git add .
git commit -m "과제: Supabase 연결 설정 + 테이블 생성"
git push
```

④ **배포 확인**: Vercel에서 배포가 성공하는지 확인한다. 환경 변수가 제대로 등록되었다면 빌드 에러 없이 배포된다.

_전체 프로젝트는 practice/chapter8/ 참고_

### 과제 제출 + 마무리

- 배포 URL + GitHub 저장소 링크 제출
- 제출 확인
- **Supabase 대시보드 스크린샷** (Table Editor에서 테이블 2개 보이는 화면) 함께 제출

---

## 장 요약

- **BaaS**(Backend as a Service)는 백엔드를 직접 만들지 않고 서비스로 사용하는 방식이다
- **Supabase**는 PostgreSQL 기반 오픈소스 BaaS — SQL을 배울 수 있고 데이터 이전이 자유롭다
- 프로젝트 생성 시 **Database Password**를 반드시 메모한다 (변경 불가)
- **anon key**는 클라이언트용 공개 키, **service_role key**는 서버 전용 비밀 키
- `@supabase/ssr`의 `createBrowserClient`로 Next.js App Router에서 클라이언트를 초기화한다
- 환경 변수는 `.env.local`에 저장하고, `NEXT_PUBLIC_` 접두사를 반드시 붙인다
- Vercel 배포 시 대시보드에서 **환경 변수를 별도 등록**해야 한다
- **관계형 데이터베이스**는 데이터를 테이블(표) 형태로 저장한다
- **1:N 관계**: 한 사용자가 여러 게시글을 작성 — `posts.user_id`가 `profiles.id`를 참조
- `profiles` 테이블은 `auth.users`와 연결하는 Supabase 공식 패턴이다
...
### 수업 후 체크
- [ ] 전원 Supabase 프로젝트 생성 완료 확인
- [ ] 전원 `.env.local` 설정 + `lib/supabase.js` 파일 생성 확인
- [ ] 전원 Vercel 환경 변수 등록 확인
- [ ] 전원 `profiles` + `posts` 테이블 생성 확인
- [ ] 전원 배포 URL 제출 확인
- [ ] 미제출자 파악 및 개별 안내
- [ ] service_role key를 `.env.local`에 넣은 학생이 있는지 점검 (있다면 삭제 지도)

---

## 다음 장 예고

Supabase가 연결되었고 테이블도 만들었다. 하지만 아직 **누가 글을 쓰는지** 구분할 수 없다. Ch9에서는 **Google 계정으로 로그인**하는 기능을 구현한다. Supabase Authentication과 OAuth를 사용하여, 로그인한 사용자만 게시글을 작성할 수 있게 만든다.
