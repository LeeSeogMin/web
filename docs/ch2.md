...
| 시간 | 구분 | 내용 |
|------|------|------|
| 00:00~00:50 | **1교시** | 바이브코딩 + AI 3대 한계 |
| 00:50~01:00 | 쉬는시간 | |
| 01:00~01:50 | **2교시** | Copilot 설치 + copilot-instructions.md 작성 |
| 01:50~02:00 | 쉬는시간 | |
| 02:00~02:50 | **3교시** | 자기소개 페이지 생성 + 배포 + 과제 제출 |
...
> **강의 팁**: Copilot에게 라이브로 Next.js 코드를 요청하여 `next/router` vs `next/navigation` 차이를 직접 시연한다.
...
- 컨텍스트 문제 → copilot-instructions.md (2교시에 직접 작성)
- 버전 문제 → 버전 확인 + copilot-instructions.md 기록 + Context7 MCP
- 환각 문제 → 검증 체크리스트 (3교시에 실습)
- **핵심 메시지**: "AI를 잘 쓰려면 AI의 한계를 알아야 한다"
...
> Tailwind CSS로 스타일링하고, 왼쪽에 사이트 제목 '게시판', 오른쪽에 '글 목록'과 '로그인' 링크를 배치해줘.
...
> MCP 초기 설정은 한 번만 하면 된다. 자세한 방법은 부록 D를 참고한다. **설정이 가능한 학생은 적극 권장한다.**
...
2. `.github/copilot-instructions.md` 파일 작성 완료
3. Copilot Chat을 사용하여 **자기소개 페이지** 생성
4. **AI 사용 로그** 작성
...
<!-- COPILOT_VERIFY: 위 프롬프트의 실제 Copilot 응답 캡처 -->
...
본인 정보로 내용 수정. 추가 프롬프트로 기능 추가 가능.

**④ 배포**

```bash
git add .
git commit -m "과제: 자기소개 페이지 + copilot-instructions.md"
git push
```
...
**제출물**:
- 배포된 Vercel URL
- GitHub 저장소 (copilot-instructions.md 포함)
- AI 사용 로그
...
### 준비물
- [ ] GitHub Education 사전 안내 (1주차에 공지)
- [ ] Copilot 설치 트러블슈팅 가이드
- [ ] copilot-instructions.md 전/후 비교 시연용 프로젝트
- [ ] 과제 제출 방법 안내 (LMS 설정)

### 수업 후 체크
- [ ] 전원 Copilot 설치 완료 확인
- [ ] 전원 copilot-instructions.md 작성 확인
- [ ] 전원 배포 URL 제출 확인
...
다음 장에서는 **HTML 시맨틱과 Tailwind CSS**를 배운다. 오늘 설정한 Copilot과 함께, 게시판 UI를 마크업하고 스타일링하는 실습을 진행한다.
