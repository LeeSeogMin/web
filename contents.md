...
|:---:|:---:|------|------|------|
| 1 | Ch1 | 웹의 구조 + 환경 설정 + 첫 배포 시연 | 블로그 첫 페이지 만들기 + 배포 | 배포 URL |
| 2 | Ch2 | AI 3대 한계 + Copilot 세팅 + 바이브코딩 시연 | copilot-instructions.md + 블로그 소개 페이지 | 배포 URL |
| 3 | Ch3 | HTML 시맨틱 + Tailwind CSS 강의 | 블로그 레이아웃 마크업 + 스타일링 | 배포 URL |
| 4 | Ch4 | JavaScript 핵심 강의 | 블로그에 JS 인터랙션 추가 | 배포 URL |
| 5 | Ch5 | Next.js App Router 강의 | 블로그 글 목록/상세/작성 페이지 | 배포 URL |
| 6 | Ch6 | 상태 관리 + 데이터 페칭 강의 | 블로그 프론트엔드 완성 | 배포 URL |
| 7 | Ch7 | 아키텍처 + AI 디자인 설계 강의 | 블로그 아키텍처 설계 보완 | 설계서 제출 |
| **8** | — | **중간고사 대비** | **중간고사** | **코딩 객관식** |
| 9 | Ch8 | Supabase 기초 + DB 연결 강의 | 블로그에 Supabase 연결 (더미→DB) | 배포 URL |
| 10 | Ch9 | 인증 개념 + OAuth 강의 | 블로그 로그인 구현 | 배포 URL |
| 11 | Ch10 | SQL + CRUD 강의 | 블로그 글 CRUD 구현 | 배포 URL |
| 12 | Ch11 | RLS 보안 강의 | 블로그 보안 정책 설정 + 검증 | 배포 URL |
| 13 | Ch12 | 에러 처리 + UX 강의 | 블로그 UX 완성 | 배포 URL |
| 14 | Ch13 | 블로그 확장 기능 가이드 + 워크숍 | 블로그 확장 기능 구현 (댓글, 태그, 검색) | 배포 URL |
...
1.6 과제: 블로그 첫 페이지 만들기 → git push → 배포 URL 제출

### Chapter 2. Copilot 세팅과 바이브코딩

> **미션**: Copilot을 제대로 세팅하고, 말로 설명해서 블로그 소개 페이지를 만든다
...
> **라이브 코딩 시연**: 교수가 copilot-instructions.md를 작성하고, Copilot에게 블로그 소개 페이지를 생성시키는 과정을 시연한다.
...
**B회차: Copilot 설정 + 블로그 소개 페이지**

2.9 과제: Copilot + MCP 설정 완료 + copilot-instructions.md 작성 + 블로그 소개 페이지 만들기 + 배포
...
> **라이브 코딩 시연**: 교수가 블로그 레이아웃을 Tailwind로 구현하는 과정을 시연한다.
...
3.6 과제: 블로그 레이아웃 마크업 + Tailwind 스타일링 + 배포
...
> **라이브 코딩 시연**: 교수가 블로그 글 목록/상세 페이지를 App Router로 구현하는 과정을 시연한다.
...
5.5 과제: 블로그 글 목록/상세/작성 페이지 구현 + 배포
...
> **라이브 코딩 시연**: 교수가 블로그에 상태 관리와 데이터 페칭을 추가하는 과정을 시연한다.
...
> **미션**: 블로그 아키텍처를 설계하고 보완한다 — shadcn/ui, 디자인 토큰, DB 스키마가 바이브코딩의 품질을 결정한다
...
**B회차: 블로그 아키텍처 설계 보완 실습**

7.7 과제: 블로그 아키텍처 설계서 작성
    (페이지 맵 + AI 와이어프레임 + shadcn/ui 테마 설정 + DB 스키마 + copilot-instructions.md + ARCHITECTURE.md)
...
> **미션**: 블로그의 더미 데이터를 Supabase 데이터베이스로 교체한다
...
8.4 데이터 모델링
  ...
  - 8.4.2 7장 설계서를 기반으로 posts 테이블 생성
  - 8.4.3 테이블 관계: 1:N (사용자 → 블로그 글)
  - 8.4.4 SQL로 테이블 생성
  - 8.4.5 더미 데이터(lib/posts.js)를 Supabase 쿼리로 교체

**B회차: 블로그에 Supabase 연동 실습**

8.5 과제: 블로그에 Supabase 연결 + 더미→DB 교체 + 배포

### Chapter 9. Supabase Authentication

> **미션**: 블로그에 로그인 기능을 추가한다
...
> **라이브 코딩 시연**: 교수가 블로그 글 CRUD 전 과정을 Supabase 클라이언트로 구현한다.
...
**B회차: 블로그 글 CRUD 구현 실습**

10.5 과제: 블로그 글 CRUD 완성 + 배포
...
### Chapter 13. 블로그 확장 기능 구현

> **미션**: 블로그에 댓글, 태그, 검색 기능을 추가하고 완성한다

**A회차: 블로그 확장 기능 가이드 + 워크숍**

13.1 확장 기능 요구사항 확인
  - 13.1.1 Ch12까지 완성된 블로그 점검
  - 13.1.2 확장 기능 우선순위 정리 (댓글, 태그, 검색)
  - 13.1.3 MVP(최소 기능 제품) 범위 확정
13.2 바이브코딩으로 확장 기능 구현
...
| 항목 | 내용 |
|------|------|
| A회차 | **블로그 최종 발표 + 제출** (개인 시연 + 배포 URL + GitHub + AI 로그) |
...
```markdown
## 기능 명세
- 블로그 글 CRUD: 로그인 사용자만 작성, 작성자만 수정/삭제
- 소셜 로그인: Google OAuth
